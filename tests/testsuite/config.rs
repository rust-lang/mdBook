...
// Check that the `zoomable-images` setting is working as expected.
#[test]
fn zoomable_images() {
    const ZOOMABLE_IMG_MARKER: &str = "<input class=\"checkbox-img\" type=\"checkbox\">";
    // By default, `zoomable-images` is enabled, so the zoomable images feature "marker"
    ...
    BookTest::from_dir("config/empty")
        .change_file("book.toml", "[output.html]\nzoomable-images = false")
        .run("build", |_cmd| {})
