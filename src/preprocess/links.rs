...
impl<'a> Link<'a> {
    ...
    fn render_with_path<P: AsRef<Path>>(&self, base: P) -> Result<String> {
        ...
        match self.link {
            ...
            LinkType::Include(ref pat) => file_to_string(base.join(pat)).chain_err(|| format!("Could not read file for link {}", self.link_text)),
            LinkType::Playpen(ref pat, ref attrs) => {
                let contents = file_to_string(base.join(pat)).chain_err(|| format!("Could not read file for link {}", self.link_text))?;
