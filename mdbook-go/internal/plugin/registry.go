...
func parsePreprocessorConfig(cfg *config.Config) map[string]PreprocessorConfigEntry {
...
func BuildPreprocessors(cfg *config.Config, root string) ([]Preprocessor, error) {
...
func buildOne(name string, table map[string]PreprocessorConfigEntry, root string) Preprocessor {
	switch name {
	case "links":
...
// renderer is provided by internal/render; this helper only handles the
// custom command renderers the user registers.
func BuildRenderers(cfg *config.Config) (map[string]Renderer, error) {
	out := map[string]Renderer{}
	for name, raw := range cfg.Output {
		if name == "html" || name == "markdown" {
			continue // handled by the driver
		}
		cmd := "mdbook-" + name
		if tbl, ok := raw.(map[string]any); ok {
...
func ShouldRunPreprocessor(p Preprocessor, renderer Renderer, cfg *config.Config) (bool, error) {
	name := p.Name()
...
func RunPreprocessors(pre []Preprocessor, cfg *config.Config, root string, renderer string, b *book.Book) (*book.Book, *PreprocessorContext, error) {
	wc := ToWireConfig(cfg)
	ctx := &PreprocessorContext{
		Root:          root,
		Config:        NewPreprocessorConfig(wc),
		Renderer:      renderer,
		MdbookVersion: MdbookVersion,
