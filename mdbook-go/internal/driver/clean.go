...
// Clean removes the build directory rooted at the book's configured
// build-dir, or at destDir when provided. When destDir is empty, the
// MDBook's BuildDir() is used; this matches `mdbook clean --dest-dir <dir>`
// in the Rust CLI.
func (m *MDBook) Clean(destDir string) (*Clean, error) {
	dir := destDir
	if dir == "" {
		dir = m.BuildDir()
	}
...
// RemoveDir deletes a directory tree and returns a Clean summary. It is
// the no-book counterpart used by `mdbook clean --dest-dir <path>` so the
// CLI does not need to load a valid book just to wipe a custom output
// directory. A non-existent path returns zero stats without an error.
func RemoveDir(dir string) (*Clean, error) {
	return removeBuildDir(dir)
}
