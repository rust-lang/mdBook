...
// renderTocSidebar is the production-side analogue of render/toc.go:tocHelper.
// It reproduces the byte-level output of the hbs `{{#toc}}` block helper so
// the templates see a pre-computed string via {{.TocHTML}}.
//
// This is a re-implementation of the helper logic (rather than a call to
// internal/render.tocHelper) so that the tplgotpl package does not depend on
// internal/render — that would create an import cycle.
func renderTocSidebar(chapters []any, foldEnable bool, foldLevel int, noSectionLabel bool, isTocHTML bool) string {
	...
	for _, raw := range chapters {
		...
		if section, ok := item["section"]; ok && section != "" {
			itemLevel = strings.Count(section, ".") + 1
		}
...
// keep strconv import live — used in helpers.go for asInt via direct number
// coercion in templates.
var _ = strconv.Itoa

// SidebarHeaderNavSource is the IIFE the original toc.js.hbs emitted inside
// `{{#if sidebar_header_nav}}…{{/if}}`. We expose it as a constant so callers
// (Env.SidebarHeaderNavBlocks) can splice it into toc.js without a template
// round-trip.
//
// Source kept verbatim from theme/templates/toc.js.hbs lines 79-456.
