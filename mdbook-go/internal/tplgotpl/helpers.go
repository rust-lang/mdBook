...
// the closures built in internal/render/render.go: resources are only known
// after static files are written, and the `path` of the current page influences
// how many `../` the `resource` helper emits.
type Env struct {
	...
	// header tracking, kept verbatim from toc.js.hbs. RenderTocJS decides
	// whether to emit it based on SidebarHeaderNav.
	...
	// template.JS so it survives the JS string literal in index.gohtml
	// unescaped.
...
// `{{#if sidebar_header_nav}}…{{/if}}` emitted. We pre-stash it via
...
// the given page path. Mirrors utils.PathToRoot.
...
// asInt coerces v to int, matching utils/PathToRoot-style permissive parsing.
