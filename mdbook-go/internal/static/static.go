// Package static assembles, fingerprints and writes the theme's static assets.
...
// additional-js paths.
func New(t *theme.Theme, cfg *config.HtmlConfig, root string) (*Files, error) {
	...
	if t.FaviconSVG != nil {
		f.AddBuiltin("favicon.svg", t.FaviconSVG)
	}
	...
	if t.FontsCSS == nil {
		...
		for _, license := range theme.FontLicenses {
			f.AddBuiltin(license.Name, license.Data)
		}
		for _, font := range theme.OpenSans {
			f.AddBuiltin(font.Name, font.Data)
		}
	...
	for _, fontFile := range t.FontFiles {
		f.addAdditional("fonts/"+filepath.Base(fontFile), fontFile)
	}
	return f, nil
}
...
func (f *Files) Write(destination string) (map[string]string, error) {
	...
	for _, file := range f.files {
		...
		if !rewritable {
			...
			} else if err := utils.CopyFile(file.sourcePath, out); err != nil {
				return nil, err
			}
			continue
		}
...
func (f *Files) replaceAll(data []byte, destination string) []byte {
	pathToRoot := utils.PathToRoot(destination)
