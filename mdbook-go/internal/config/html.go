...
func (c *Config) HTML() (*HtmlConfig, error) {
	...
	// Round-trip through TOML so absent keys keep their default rather than
	// being zeroed, which is what serde's #[serde(default)] does.
	encoded, err := toml.Marshal(map[string]any{"html": raw})
	if err != nil {
		return nil, err
	}
	var wrapper struct {
	...
	if cfg.Search != nil {
		...
		if sub, ok := subTable(raw, "search"); ok {
			encoded, err := toml.Marshal(map[string]any{"search": sub})
			if err != nil {
				return nil, err
			}
			var sw struct {
			...
			sw.Search = &merged
			if err := toml.Unmarshal(encoded, &sw); err != nil {
				return nil, err
			}
		}
		cfg.Search = &merged
...
func (b *BookConfig) RealizedTextDirection() string {
	if b.TextDirection != "" {
