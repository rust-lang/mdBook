...
`book.toml` declares `examples/` as an extra-watch-dir so a change
...
```bash
go run ./cmd/mdbook watch -dir fixtures/serve -watcher poll
# in another shell:
...
```bash
go run ./cmd/mdbook watch -dir fixtures/serve -watcher native
```

### serve

```bash
go run ./cmd/mdbook serve -dir fixtures/serve -port 3000
...
```bash
go run ./cmd/mdbook serve -dir fixtures/serve -open
